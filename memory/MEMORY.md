...
- [The contract master loads once per broker](contract-master-loads-once-per-broker.md) — a lock per broker plus a 60s negative cache; one user's slow broker no longer stalls everyone else's request.
